-- CREATE SCHEMA VOOS AUTHORIZATION "VicenteCosta"
	...
	CREATE Table Reservas_de_Voos_Fare(
		...
		Foreign key(Flight_Number) REFERENCES Reservas_de_Voos_Flight(number),
		check(amount >= 0)
	);
	...
	CREATE Table Reservas_de_Voos_Flight_leg(
		...
		Foreign key(Flight_Number) REFERENCES Reservas_de_Voos_Flight(number),
		Foreign key(Airport1) REFERENCES Reservas_de_Voos_Airport(Airport_code),
		Foreign key(Airport2) REFERENCES Reservas_de_Voos_Airport(Airport_code)
	);
...
;
	CREATE Table Reservas_de_Voos_Leg_instance(
		...
		foreign key(flight_number)	REFERENCES Reservas_de_Voos_Flight_leg (Flight_Number),
		foreign key(flight_leg_no)	REFERENCES Reservas_de_Voos_Flight_leg (leg_no),
	...
	CREATE table Reservas_de_Voos_Can_land(
		...
		foreign key (typename) References Reservas_de_Voos_Airplane_type(typename),
		foreign key (Airport_Code) References Reservas_de_Voos_Airport(Airport_code),
...
;
	CREATE table Reservas_de_Voos_Airplane(
		...
		foreign key (AiT_type_name) References Reservas_de_Voos_Airplane_type (typename),
	...
	CREATE table Reservas_de_Voos_Seat(
		...
		foreign key (Leg_instance_date) references Reservas_de_Voos_Leg_instance([date]),
		foreign key (Flight_number) references Reservas_de_Voos_Leg_instance(Flight_number),
		foreign key (flight_leg_num) references Reservas_de_Voos_Leg_instance(flight_leg_num),
